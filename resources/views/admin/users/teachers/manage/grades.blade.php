        ...
        <div class="card-datatable table-responsive pt-0">
            <div class="card-header d-flex align-items-center justify-content-between flex-column flex-md-row border-bottom">
                <div class="head-label text-center">
                    <h5 class="card-title mb-0">{{ trans('main.datatableTitle', ['item' => trans('admin/grades.grades')]) }} - {{ $teacher->name }}</h5>
                </div>
            </div>
        ...
        $(document).ready(function() {
            $('.accordion-button').on('click', function() {
                ...
                if (!accordionBody.hasClass('show') && !$.fn.DataTable.isDataTable(tableId)) {
                    initializeDataTable(
                        tableId,
                        "{{ route('admin.teachers.grades.groups', ['teacherId' => ':teacherId', 'gradeId' => ':gradeId']) }}".replace(':teacherId', teacherId).replace(':gradeId', gradeId),
