...
@section('content')
    ...
    @include('admin.finance.studentFees.modals')
    <!--/ DataTable with Buttons -->
@endsection

@section('page-js')
    <script>
        initializeDataTable('#datatable', "{{ route('admin.student-fees.index') }}", [2, 3, 4, 5, 6, 7],
            [{
                    data: "",
                    orderable: false,
                    searchable: false
        ...
        fetchMultipleDataByAjax('#add-form #student_id', "{{ route('admin.fetch.students.fees', '__ID__') }}",
            '#add-form #fee_id', 'student_id', 'GET');
        fetchSingleDataByAjax('#add-form #fee_id', "{{ route('admin.fetch.fees.data', '__ID__') }}", [
