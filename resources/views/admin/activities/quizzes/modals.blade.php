<!-- Add Modal -->
<x-modal modalType="add" modalSize="modal-lg" modalTitle="{{ trans('main.addItem', ['item' => trans('admin/quizzes.quiz')]) }}" action="{{ route('admin.quizzes.insert') }}">
    <div class="row g-5">
        ...
        <x-select-input divClasses="col-12 col-md-4" name="quiz_mode" label="{{ trans('main.status') }}" :options="[1 => trans('admin/quizzes.fixed'), 2 => trans('admin/quizzes.flexible')]" required/>
        <x-basic-input divClasses="col-12 col-md-4" type="text" name="start_time" classes="flatpickr-date-time" label="{{ trans('main.start_time') }}" placeholder="YYYY-MM-DD" required/>
        <x-basic-input divClasses="col-12 col-md-4" type="text" name="end_time" classes="flatpickr-date-time" label="{{ trans('main.end_time') }}" placeholder="YYYY-MM-DD" required/>
...
<!-- Edit Modal -->
<x-modal modalType="edit" modalSize="modal-lg" modalTitle="{{ trans('main.editItem', ['item' => trans('admin/quizzes.quiz')]) }}" action="{{ route('admin.quizzes.update') }}" id>
    <div class="row g-5">
        <x-basic-input context="modal" type="text" name="name_ar" label="{{ trans('main.name_ar') }}" placeholder="{{ trans('admin/quizzes.placeholders.name_ar') }}" required/>
        <x-basic-input context="modal" type="text" name="name_en" label="{{ trans('main.name_en') }}" placeholder="{{ trans('admin/quizzes.placeholders.name_en') }}" required/>
        <x-select-input context="modal" name="teacher_id" label="{{ trans('main.teacher') }}" :options="$teachers" required/>
...
<x-modal modalType="delete" modalTitle="{{ trans('main.deleteItem', ['item' => trans('admin/quizzes.quiz')]) }}"
    action="{{ route('admin.quizzes.delete') }}" id submitColor="danger" submitButton="{{ trans('main.yes_delete') }}">
    @include('partials.delete-modal-body')
</x-modal>
<!-- Delete Selected Modal -->
<x-modal modalType="delete-selected" modalTitle="{{ trans('main.deleteItem', ['item' => trans('admin/quizzes.selectedQuizzes')]) }}"
    action="{{ route('admin.quizzes.deleteSelected') }}" ids submitColor="danger" submitButton="{{ trans('main.yes_delete') }}">
    @include('partials.delete-modal-body')
</x-modal>
